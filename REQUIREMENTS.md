...
- **SCAI-REQ-002 (Artifact Generation):** Upon test suite exit, the library MUST write a singular Markdown document to the location defined by `SCAI-REQ-027` (by default `ai_report.md` inside SimpleCov's coverage directory) and announce that location on standard output as defined by `SCAI-REQ-028`. If configured to do so (`output_to_console`), the formatter MUST instead safely echo the finalized digest string to `STDOUT` for immediate human readability in a terminal; the file is written in either case. *Revised in 0.11.0: the default location now follows `SimpleCov.coverage_dir` and the write is announced (BUG-SCAI-018).*
...
- **SCAI-REQ-028 (STDOUT Notice):** After writing the report the formatter MUST print `AI coverage digest written to <absolute path>` on `STDOUT`, unless `output_to_console` is set, in which case it MUST print the digest itself instead. *Introduced in 0.11.0 so the artifact's location is discoverable from the test log.*
